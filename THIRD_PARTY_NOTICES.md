# Third-party data notices

`addons/oddcast/weakness_data.lua`, its split `weakness_data/*.lua` zone files,
and `weakness_data_manifest.json` are generated from monster spawn, pool,
resistance, modifier, zone, spell-formula, enum, and script metadata in the
CatsEyeXI/LandSandBoat server source.

- Upstream: <https://github.com/CatsAndBoats/catseyexi>
- Source commit: `4cf9796860e4a1fd338df15ee9b45406678400b9`
- Data license: GNU General Public License version 3 or later
- License text: [`LICENSE-DATA-GPL-3.0`](LICENSE-DATA-GPL-3.0)

The generated index and manifest embed an aggregate SHA-256 identity for every
material input used by the generator. OddCast's handwritten addon and tooling
remain available under the repository's MIT license; the generated
server-derived dataset is distributed under the upstream GPL terms described
above.
