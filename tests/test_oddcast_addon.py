...
def test_luashitacast_vana_time_adaptation_has_complete_attribution() -> None:
    ...
    assert "addon.version = '1.1.3';" in addon_text
    ...
    driver.write_text(
        "\n".join(
            (
                ...
                "},bits={unpack_be=function(data,base,offset,length) if base==10 and offset==2 and length==4 then return data.category end if base==0 and offset==213 and length==17 then return data.spellId end error('unexpected packet bit field') end}}",
                "local target={GetIsSubTargetActive=function() return subTargetActive end,GetTargetIndex=function(_,slot) if slot==1 then return btIndex end return mainIndex end}",
                "local entity={",
                ...
                "local castbar={GetCount=function() return castCount end}",
                "local memory={GetTarget=function() return target end,GetEntity=function() return entity end,GetPlayer=function() return player end,GetParty=function() return party end,GetRecast=function() return recast end,GetCastBar=function() return castbar end}",
                "local resourceManager={GetSpellById=function(_,id) return resources[id] end,GetEntityIndexById=function(_,id) if id==mainServerId then return mainIndex end if id==btServerId then return btIndex end if id==resolvedServerId then return resolvedIndex end if id==nonMonsterServerId then return nonMonsterIndex end return 0 end}",
                "local chatManager={QueueCommand=function(_,mode,command) queued[#queued+1]={mode=mode,command=command} end}",
                "AshitaCore={GetMemoryManager=function() return memory end,GetResourceManager=function() return resourceManager end,GetChatManager=function() return chatManager end}",
                "addon={path='fixture/'}",
                "realDofile(ODDCAST_PATH)",
                ...
                "assert(#queued==4 and queued[4].command=='/ma \"Fire V\" 333333','Multisend-resolved server ID did not select and bind the remote target')",
                "callbacks.packet_in({id=0x028,data={actorId=playerServerId},data_raw={category=8,spellId=148}})",
                "invoke('/oc','day',tostring(resolvedServerId))",
                "assert(#queued==5 and queued[5].command=='/ma \"Water V\" 333333','day did not accept the MultiSend-resolved server ID')",
                "callbacks.packet_in({id=0x028,data={actorId=playerServerId},data_raw={category=8,spellId=173}})",
                ...
                "invoke('/oc','weak','[T]')",
                "assert(#queued==6 and queued[6].command=='/ma \"Fire V\" 111111','direct weak [t] did not capture the local main target as a server ID')",
                "callbacks.packet_in({id=0x028,data={actorId=playerServerId},data_raw={category=8,spellId=148}})",
                "subTargetActive=1",
                "invoke('/oc','day','[t]')",
                "assert(#queued==7 and queued[7].command=='/ma \"Water V\" 222222','direct day [t] did not capture the active subtarget as a server ID')",
                "callbacks.packet_in({id=0x028,data={actorId=playerServerId},data_raw={category=8,spellId=173}})",
                "subTargetActive=0",
                "invoke('/oc','weak','0')",
                ...
                "invoke('/oc','weak',tostring(resolvedServerId))",
                "assert(#queued==8 and queued[8].command=='/ma \"Fire V\" 333333','a corrupt default target blocked an explicit MultiSend-resolved target')",
                "callbacks.packet_in({id=0x028,data={actorId=playerServerId},data_raw={category=8,spellId=148}})",
                "castCount=50",
                "invoke('/oc','weak',tostring(resolvedServerId))",
                ...
                "now=now+0.11",
                "callbacks.d3d_present()",
                "assert(#queued==9 and queued[9].command=='/ma \"Fire V\" 333333','target-setting change canceled or retargeted the pending explicit-ID request')",
                "now=now+2.1",
                "callbacks.d3d_present()",
                "assert(#queued==9,'explicit-ID request retried before the bounded retry lock')",
                "now=now+1.2",
                "callbacks.d3d_present()",
                "assert(#queued==10 and queued[10].command=='/ma \"Fire V\" 333333','retry did not preserve the explicit target server ID')",
                "callbacks.packet_in({id=0x028,data={actorId=playerServerId},data_raw={category=8,spellId=148}})",
                "castCount=0",
                "settingsCallback({target='<stnpc>',dayTierCeiling=5,weaknessTierCeiling=5,showRoutineChat=true})",
                "invoke('/oc','settings')",
