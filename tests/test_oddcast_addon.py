    ...
    driver.write_text(
        "\n".join(
            (
                ...
                "invoke('/oc','weak',tostring(resolvedServerId))",
                "assert(#queued==6 and queued[6].command=='/ma \"Fire V\" 333333','a corrupt default target blocked an explicit MultiSend-resolved target')",
                "callbacks.packet_in({id=0x028,data={actorId=playerServerId},data_raw={category=8,spellId=148}})",
                "castCount=50",
                "invoke('/oc','weak',tostring(resolvedServerId))",
                ...
                "now=now+2.1",
                "callbacks.d3d_present()",
                "assert(#queued==7,'explicit-ID request retried before the bounded retry lock')",
                "now=now+1.2",
                "callbacks.d3d_present()",
